...
import { Clock, ExternalLink, TrendingUp, ChevronRight, Rss, Trophy, Dumbbell, Bike, Target, Swords } from 'lucide-react';
...
export const metadata: Metadata = {
    ...
    description: 'Catch the latest sports news covering cricket, football, hockey, tennis, badminton, motorsport, wrestling, and more from top Indian sources.',
    openGraph: {
        title: 'Sports News — Belgaum Today',
        description: 'Latest sports news — Cricket, Football, Hockey, Tennis & more',
        type: 'website',
    },
};
...
const SUB_CATEGORIES = [
    { name: 'All Sports', icon: Trophy, active: true },
    { name: 'Cricket', icon: Target },
    { name: 'Football', icon: Dumbbell },
    { name: 'Tennis', icon: Swords },
    { name: 'Motorsport', icon: Bike },
];
...
export default async function SportsNewsPage() {
    ...
    try {
        trendingArticles = await query<TrendingArticle[]>(
            `SELECT a.id, a.title, a.excerpt, a.featured_image, a.source_name, a.source_url, a.published_at,
             ...
             WHERE t.category = 'sports' AND a.status = 'published'
        ...
        articles = await query<Article[]>(
            `SELECT * FROM articles WHERE category = 'sports' AND status = 'published' ${excludeClause} ORDER BY published_at DESC LIMIT 50`
        );
        ...
        const feedStatus = await query<{ last_fetched_at: Date }[]>(
            `SELECT last_fetched_at FROM rss_feed_config WHERE category = 'sports' AND is_active = true ORDER BY last_fetched_at DESC LIMIT 1`
        );
    ...
    return (
        <TrackingProvider category="sports">
            <main className="min-h-screen bg-gray-50 dark:bg-gray-950">
                {/* Header — Red/Orange theme */}
                <section className="bg-gradient-to-r from-red-700 via-red-600 to-orange-500 dark:from-red-900 dark:via-red-800 dark:to-orange-700">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h1 className="text-2xl md:text-3xl font-bold text-white">Sports News</h1>
                                    <p className="text-red-100 text-sm mt-0.5">Cricket, Football, Hockey, Tennis & More</p>
                                </div>
                            </div>
                            <div className="hidden sm:flex items-center gap-4 text-sm text-red-100">
                                {lastUpdated && (
                        ...
                        <div className="mt-4 flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
                            {SUB_CATEGORIES.map((cat) => {
                                const Icon = cat.icon;
                                return (
                                    <span key={cat.name} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${cat.active ? 'bg-white text-red-700' : 'bg-white/10 text-white/80 hover:bg-white/20'}`}>
                ...
                {!hasNews ? (
                    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
                        <div className="text-center">
                            ...
                            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-3">No Sports News Yet</h2>
                            <p className="text-gray-500 dark:text-gray-400 max-w-md mx-auto">Sports news articles are being fetched. Check back shortly.</p>
                        </div>
                    </section>
                ) : (
                        ...
                        <div className="lg:grid lg:grid-cols-12 lg:gap-8">
                            <div className="lg:col-span-8 xl:col-span-9">
                                {trendingArticles.length > 0 && (
                                    <div className="mb-8">
                                        <TrendingCarousel articles={trendingArticles} accentColor="red" />
                                    </div>
                                )}
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                                    {articles.map((article, index) => (
                                        <a key={article.id} href={article.source_url} target="_blank" rel="noopener noreferrer" data-article-id={article.id} data-source-name={article.source_name} className="group bg-white dark:bg-gray-900 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-800 transition-all duration-300 hover:shadow-xl hover:scale-[1.02] hover:border-red-400/50">
                                            ...
                                            <div className="p-4">
                                                <h3 className="text-base font-semibold text-gray-900 dark:text-white mb-2 line-clamp-2 group-hover:text-red-600 dark:group-hover:text-red-400 transition-colors leading-snug">{article.title}</h3>
                                                ...
                                                <div className="flex items-center justify-between text-xs text-gray-400">
                                                    ...
                                                    <span className="flex items-center gap-1 text-red-500 font-medium opacity-0 group-hover:opacity-100 transition-opacity">Read <ChevronRight className="w-3 h-3" /></span>
                                                </div>
                                            </div>
                                        </a>
                                    ))}
                                ...
                                <div className="sticky top-24 space-y-6">
                                    <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
                                        <h3 className="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider mb-4 flex items-center gap-2">
                                            <Rss className="w-4 h-4 text-red-500" /> News Sources
                                        </h3>
                                    ...
                                    <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
                                        ...
                                        <div className="space-y-1">
                                            {[
                                                ...
                                                <Link key={cat.slug} href={`/${cat.slug}`} className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-gray-600 dark:text-gray-400 hover:bg-red-50 dark:hover:bg-gray-800 hover:text-red-600 dark:hover:text-red-400 transition-colors">
                                    ...
                                    <div className="bg-gradient-to-br from-red-600 to-orange-600 rounded-xl p-5 text-white">
                                        <h3 className="font-bold mb-2 flex items-center gap-2"><TrendingUp className="w-4 h-4" /> Sports Arena</h3>
                                        <p className="text-sm text-red-100 mb-3 leading-relaxed">Sports news from 19 feeds covering cricket, football, hockey, tennis, badminton, motorsport, and more.</p>
                                        <div className="flex items-center gap-2 text-xs text-red-200"><Rss className="w-3.5 h-3.5" /> 19 RSS feeds • Auto-updated</div>
                                    </div>
                                </div>
                            </aside>
                        </div>
