...
import { Clock, ExternalLink, TrendingUp, ChevronRight, Rss, Cpu, Monitor, Smartphone, Wifi, Globe } from 'lucide-react';
...
export const metadata: Metadata = {
    title: 'Technology News — Latest Tech, Science & Innovation',
    description: 'Stay updated with the latest technology news covering AI, gadgets, science, startups, and digital innovation from Hindustan Times and The Hindu.',
    openGraph: {
        title: 'Technology News — Belgaum Today',
        description: 'Latest technology news — AI, Gadgets, Science & Innovation',
        type: 'website',
    },
};
...
const SUB_CATEGORIES = [
    { name: 'All Tech', icon: Cpu, active: true },
    { name: 'AI & ML', icon: Monitor },
    { name: 'Gadgets', icon: Smartphone },
    { name: 'Internet', icon: Wifi },
    { name: 'Science', icon: Globe },
];
...
export default async function TechnologyNewsPage() {
    ...
    try {
        trendingArticles = await query<TrendingArticle[]>(
            `SELECT a.id, a.title, a.excerpt, a.featured_image, a.source_name, a.source_url, a.published_at,
             ...
             WHERE t.category = 'technology' AND a.status = 'published'
        ...
        articles = await query<Article[]>(
            `SELECT * FROM articles WHERE category = 'technology' AND status = 'published' ${excludeClause} ORDER BY published_at DESC LIMIT 50`
        );
        ...
        const feedStatus = await query<{ last_fetched_at: Date }[]>(
            `SELECT last_fetched_at FROM rss_feed_config WHERE category = 'technology' AND is_active = true ORDER BY last_fetched_at DESC LIMIT 1`
        );
    ...
    return (
        <TrackingProvider category="technology">
            <main className="min-h-screen bg-gray-50 dark:bg-gray-950">
                {/* Page Header — Blue/Cyan theme */}
                <section className="bg-gradient-to-r from-blue-700 via-cyan-600 to-blue-500 dark:from-blue-900 dark:via-cyan-800 dark:to-blue-700">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h1 className="text-2xl md:text-3xl font-bold text-white">Technology News</h1>
                                    <p className="text-blue-100 text-sm mt-0.5">AI, Gadgets, Science & Innovation</p>
                                </div>
                            </div>
                ...
                {!hasNews ? (
                    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
                        <div className="text-center">
                            ...
                            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-3">No Technology News Yet</h2>
                            <p className="text-gray-500 dark:text-gray-400 max-w-md mx-auto">
                                Technology news articles are being fetched from our sources. Please check back shortly.
                            </p>
                        </div>
                    </section>
                ) : (
                        ...
                        <div className="lg:grid lg:grid-cols-12 lg:gap-8">
                            ...
                            <div className="lg:col-span-8 xl:col-span-9">
                                ...
                                {trendingArticles.length > 0 && (
                                    <div className="mb-8">
                                        <TrendingCarousel articles={trendingArticles} accentColor="blue" />
                                    </div>
                                )}
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                                    {articles.map((article, index) => (
                                        <a
                                            ...
                                            className="group bg-white dark:bg-gray-900 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-800 transition-all duration-300 hover:shadow-xl hover:scale-[1.02] hover:border-blue-400/50"
                                        >
                                            ...
                                            <div className="p-4">
                                                <h3 className="text-base font-semibold text-gray-900 dark:text-white mb-2 line-clamp-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors leading-snug">
                                                    {article.title}
                                                </h3>
                                                ...
                                                <div className="flex items-center justify-between text-xs text-gray-400">
                                                    ...
                                                    <span className="flex items-center gap-1 text-blue-500 font-medium opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <div className="sticky top-24 space-y-6">
                                        ...
                                        <div className="space-y-1">
                                            {[
                                                ...
                                                <Link
                                                    key={cat.slug}
                                                    href={`/${cat.slug}`}
                                                    className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-gray-600 dark:text-gray-400 hover:bg-blue-50 dark:hover:bg-gray-800 hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
                                                >
                                                    <span>{cat.emoji}</span>
                                                    <span>{cat.name}</span>
                                    ...
                                    <div className="bg-gradient-to-br from-blue-600 to-cyan-600 rounded-xl p-5 text-white">
                                        ...
                                        <p className="text-sm text-blue-100 mb-3 leading-relaxed">
                                            Technology news sourced from 2 specialized feeds covering AI, gadgets, science, and innovation.
                                        </p>
                                        <div className="flex items-center gap-2 text-xs text-blue-200">
                                            <Rss className="w-3.5 h-3.5" />
                                            2 RSS feeds • Auto-updated
                                        </div>
                                    </div>
                                </div>
                            </aside>
