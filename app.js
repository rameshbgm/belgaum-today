...
// ==================== ADVANCED SEARCH (CITY OPTIONAL + FALLBACK) ====================
function renderAdvancedSearch() {
    ...
    if (!existingSearchForm) {
        ...
        searchBtn.addEventListener('click', async () => {
            ...
            let query, uniqueKey, items, location;
            
            // Try with city first (if provided)
            if (city) {
                query = `${city} ${country} ${topic}`;
                uniqueKey = `${date}_${query}`;
                ...
                console.log(`🔍 Searching with city: ${query}`);
                items = await fetchNews(query, date, true);
                
                // Fallback: If no results with city, try without city
                if (items.length === 0) {
                    console.log(`⚠️ No results with city. Retrying: ${country} ${topic}`);
                    showNotification('🔄 No results with city. Searching country-wide...', 'info');
                ...
                location = items.length > 0 && city ? `${city}, ${country}` : country;
            } else {
                // No city provided, search country + topic only
                query = `${country} ${topic}`;
                uniqueKey = `${date}_${query}`;
                ...
                console.log(`🔍 Searching country-wide: ${query}`);
                items = await fetchNews(query, date, true);
                location = country;
            }
            ...
            if (items.length > 0) {
                ...
                showNotification('❌ No news found for this search', 'error');
            }
...
// ==================== CREATE NEWS CARD (UPDATED SOURCE) ====================
function createNewsCard(item) {
    ...
    // Display: Just the actual source name (e.g., "The Hindu")
...
// ==================== SETUP FONT DROPDOWN (WITH PREVIEW) ====================
function setupFontDropdown() {
    FONTS.forEach(font => {
        const option = document.createElement('option');
        option.value = font.name;
        option.textContent = font.name;
        option.style.fontFamily = font.family; // Show font in dropdown
...
function setupStickyScroll() {
    const searchSection = document.querySelector('.date-picker-section');
